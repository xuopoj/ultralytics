        ...
        if point_inputs is not None:
            ...
            if prev_out is not None and prev_out.get("pred_masks") is not None:
                prev_sam_mask_logits = prev_out["pred_masks"].to(
                    device=self.device, non_blocking=self.device.type in {"cuda", "npu"}
                )
    ...
    def _run_single_frame_inference(
        ...
        if maskmem_features is not None:
            current_out["maskmem_features"] = maskmem_features.to(
                dtype=torch.float16, device=self.device, non_blocking=self.device.type in {"cuda", "npu"}
            )
        # NOTE: Do not support the `fill_holes_in_mask_scores` function since it needs cuda extensions
        # potentially fill holes in the predicted masks
        # if self.fill_hole_area > 0:
        #     pred_masks = current_out["pred_masks"].to(self.device, non_blocking=self.device.type in {"cuda", "npu"})
...
class SAM2DynamicInteractivePredictor(SAM2Predictor):
    ...
    def update_memory(
        ...
        high_res_masks = F.interpolate(
            consolidated_out["pred_masks"].to(self.device, non_blocking=self.device.type in {"cuda", "npu"}),
            size=self.imgsz,
            mode="bilinear",
            align_corners=False,
        )
